...
namespace ADOPets.Web.ViewModels.Profile
{
    public class SelectPlanViewModel
    {
        ...
        public SelectPlanViewModel(Model.Subscription subModel)
        {
            if (subModel.PlanFeatures != null && subModel.PlanFeatures.Count() > 0)
            {
                lstPlanData = new PlanDetailsViewModel1(subModel.PlanFeatures.First(), subModel);
            }
            else
            {
                lstPlanData = new PlanDetailsViewModel1(true, subModel);
            }
        }

        [Display(Name = "Account_SignUp_Promocode", ResourceType = typeof(Wording))]
        public string Promocode { get; set; }
        public PlanDetailsViewModel1 lstPlanData;
    }

    public class PlanDetailsViewModel1
    {
        public PlanDetailsViewModel1(Model.PlanFeature planFeature, Model.Subscription subModel)
        {
        ...
        public PlanDetailsViewModel1(bool isFeature, Model.Subscription subModel)
        {
            string planTypeName = (subModel.PlanType != null) ? subModel.PlanType.Name : string.Empty;
            PlanType = planTypeName;
            Price = subModel.Amount.ToString();
