...
namespace ADOPets.Web.Common.Authentication
{
    public class CustomMembershipProvider : MembershipProvider
    {
        ...
        public override bool ValidateUser(string userName, string password)
        {
            ...
            {
                var user = uow.LoginRepository.GetByUserNameAndPassword(userName, password);
                return user != null;
            }
        }
        ...
        public override MembershipUser GetUser(string username, bool userIsOnline)
        {
            ...
            using (var uow = new UnitOfWork())
            {
                var login = uow.LoginRepository.GetByUserName(username, l => l.User.UserSubscription, l => l.User.UserSubscription.Subscription, l => l.User.UserSubscription.SubscriptionService, l => l.User.TimeZone);


                if (login == null || login.User == null)
                    return null;
                if (login.User.UserSubscriptionId != null)
                {
                    int usrSubscriptionId = Convert.ToInt32(login.User.UserSubscriptionId);
                    ICollection<PaymentHistory> paymentdetails = uow.UserSubscriptionRepository.GetSingle(u => u.Id == usrSubscriptionId, navigationProperties: u => u.PaymentHistories).PaymentHistories;
                    if (paymentdetails != null)//&& u.Subscription.Name != Constants.FreeUSerPromoCode
                    {
                        login.User.UserSubscription.PaymentHistories = paymentdetails;
                    }
                }
                var membershipUser = new CustomMembershipUser(login);
        ...
        public override string GetUserNameByEmail(string email)
        {
            using (var uow = new UnitOfWork())
            {
                var userId = uow.UserRepository.GetUserIdByEmail(email);
                if (!userId.HasValue)
                    return null;

                var login = uow.LoginRepository.GetSingle(u => u.UserId == userId.Value);
