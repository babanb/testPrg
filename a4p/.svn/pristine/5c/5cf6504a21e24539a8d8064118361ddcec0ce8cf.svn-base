...
{
    public class SubscriptionExpirationAlert
    {
        public SubscriptionExpirationAlert()
        {
            var uow = new UnitOfWork();
            var mailSender = new MailSenderHelper(ConfigurationManager.AppSettings[Constants.LogoPath], ConfigurationManager.AppSettings[Constants.SignaturePath]);
        ...
        private static void SendHsbcTenDaysBeforeSubscriptionNotification(UnitOfWork uow, MailSenderHelper mailSender)
        {
            ...
            if (noOfEmailSent > 0)
            {
                var filePath = ConfigurationManager.AppSettings[Constants.ExpiretionAlertsLogFilesPath];
